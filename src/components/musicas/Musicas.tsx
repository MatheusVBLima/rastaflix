...
export function Musicas({ initialMusicas, isAdmin }: MusicasProps) {
  ...
  const filteredMusicas = useMemo(() => {
    console.log("[Musicas.tsx] Calculando filteredMusicas. 'musicas' do useQuery:", musicas);
    if (!musicas || !Array.isArray(musicas)) {
      console.log("[Musicas.tsx] 'musicas' do useQuery é undefined ou não é um array, retornando array vazio para filteredMusicas.");
      return [];
    }
    ...
    const result = musicasList.filter((music: Music) => {
      const matchesSearchTerm = music.title.toLowerCase().includes(searchTerm.toLowerCase());
    ...
    <div className="container mx-auto py-8 px-4 md:px-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {filteredMusicas.map((music: Music) => (
          <Card key={music.id} className="flex flex-col h-full overflow-hidden group">
            <div className="flex flex-col h-full">
              ...
              <CardFooter className="p-4 pt-0 mt-auto">
                <Button asChild variant="outline" className="w-full">
                  <Link href={music.url} target="_blank" rel="noopener noreferrer">
                    Ouvir Música
                  </Link>
                </Button>
              </CardFooter>
